...
void load_datasets_and_labels(arma::mat &train_dataset, arma::Row<size_t>& train_labels, mlpack::data::DatasetInfo& info) {
    ...
    //const std::string train_path = "../../datasets_filtered/cov_train_filtrato.arff";
    //const std::string train_labels_path = "../../datasets_filtered/cov_train_labels_filtrate.csv";
    //std::cout<<"Using filtered datasets"<<std::endl;
...
double calculate_alpha(const double& total_error , const int& n_class){
    const double alpha = log((1.0 - total_error) / total_error) + log(n_class - 1);
...
void calculate_new_weights(const arma::rowvec& train_result, const double& alpha, arma::rowvec& weights){
    const arma::rowvec temp_w = arma::exp(alpha * (1.0 - train_result)) % weights;
    weights = temp_w / sum(temp_w);
}

int index_best_model(const arma::mat & trees_error){

    const arma::rowvec col_sums = sum(trees_error, 0); // Somma lungo le righe (direzione verticale)

    const arma::uword max_col_index = col_sums.index_max();
...
void load_testData_and_labels(arma::mat& testDataset, arma::Row<size_t>& test_labels,  mlpack::data::DatasetInfo& info) {
    ...
    //const std::string test_path = "../../datasets_filtered/cov_train_filtrato.arff";
    //const std::string test_labels_path = "../../datasets_filtered/cov_train_labels_filtrate.csv";
...
void accuracy_for_model(const std::vector<std::pair<mlpack::DecisionTree<>,double>>& ensemble_learning, const arma::mat& testDataset, const arma::Row<size_t>&test_labels) {
    ...
    for (const auto&[en_tree, en_alpha] : ensemble_learning) {
        ...
        const double true_predictions = static_cast<double>(arma::sum(prediction_result == 1.0));
